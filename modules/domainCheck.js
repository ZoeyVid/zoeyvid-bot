...
module.exports = {
	async checkMessageForDomains(message, config) {
		...
		if (message.content.toLowerCase().match(/discord[^\s]*gg|discord[^\s]*invite/g) != null) {
			...
			teamServerClient.send({
				content:
					...
					" hat folgende Nachricht gesendet (invite), welche automatisch gelöscht wurde ```\n" +
					message.content +
					"\n```",
			});
			message.author.send(
				`Your message was deleted since it contained a discord invite. Your message: \`\`\`\n${message.content}\n\`\`\``,
			);
		...
		for (let i = 0; i < urls.length; i++) {
			...
			await dns.lookup(urls[i], options, (_err, address) => {
				if (address === "0.0.0.0") {
					message.delete();
					teamServerClient.send({
						content:
							...
							urls[i] +
							"), welche automatisch gelöscht wurde ```\n" +
							message.content +
							"\n```",
					});
					message.author.send(
						"Your message was deleted since it contains a blocked domain (" +
							urls[i] +
							"). This bot tends to overblock, so sorry if this was the case. Your message: ```\n" +
							message.content +
							"\n```",
					);
